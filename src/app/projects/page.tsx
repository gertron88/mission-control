...
export default function ProjectsPage() {
  ...
  return (
    <DashboardLayout
      title="Projects"
      subtitle={`${projects.length} total — ${projects.filter(p => p.state === 'EXECUTING').length} executing`}
      actions={
        <button style={{
          display: 'flex',
          alignItems: 'center',
          ...
          {['All', 'EXECUTING', 'PROPOSED', 'COMPLETE', 'BLOCKED'].map((f) => (
            ...
            >
              ...
              <span style={{ marginLeft: '6px', fontSize: '10px', fontFamily: 'monospace', opacity: 0.6 }}>
                {f === 'All' ? projects.length : projects.filter(p => p.state === f).length}
              </span>
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '16px' }}>
          {filtered.map((project) => {
            const scfg = statusConfig[project.state] || statusConfig['PROPOSED'];
              ...
              <div key={project.id} style={{
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', marginBottom: '6px' }}>
                    ...
                    }}>${project.budgetSpent?.toLocaleString() || 0} / ${project.budgetAllocated?.toLocaleString() || 0}</span>
                  </div>
